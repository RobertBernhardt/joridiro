<script lang="ts">
	...
	$: contestJoined = $user?.contests?.includes(id);
				...
				<div class="button_group">
					<Button
						onClick={() => {
							...
							$contest._id = id;
							$contest.title = title;
							$contest['about_company'] = {
								name: companyName,
								logo: logo
							};
							$modal = $user ? JoinContestModal : Login;
						}}
					>
						<svg
							width="20"
							...
							xmlns="http://www.w3.org/2000/svg"
						>
							<g clip-path="url(#clip0_1_50548)">
								<path
									d="M18.3332 1.66669L9.1665 10.8334"
									stroke="white"
									stroke-width="2"
									stroke-linecap="round"
									stroke-linejoin="round"
								/>
								<path
									d="M18.3332 1.66669L12.4998 18.3334L9.1665 10.8334L1.6665 7.50002L18.3332 1.66669Z"
									stroke="white"
									stroke-width="2"
									stroke-linecap="round"
									stroke-linejoin="round"
					...
					<a href={$page.url.origin + '/contests/' + id}>
						<Button style="style2">Read More</Button>
					</a>
				</div>
			{:else}
		...
		{:else}
			<div class="contest_stats">
				<div class="participation_status">
					...
					>
						<path
							d="M0.410156 13.41L6.00016 19L7.41016 17.58L1.83016 12M22.2402 5.57996L11.6602 16.17L7.50016 12L6.07016 13.41L11.6602 19L23.6602 6.99996M18.0002 6.99996L16.5902 5.57996L10.2402 11.93L11.6602 13.34L18.0002 6.99996Z"
							fill={size == 'LARGE' ? 'white' : 'var(--red1)'}
						/>
					</svg>
					{#if id === $user._id}
						<p>You are the organizer</p>
					{:else}
						<p>You are already a part of the contest</p>
					{/if}
				</div>
				<p class="participants">
					Participants: {participants.length}
				...
				<p class="userscore">
					Your score: {calcScore(participants.find((p) => p._id === $user._id))}
				</p>
				<p class="rank">
					Your rank: #{participants.findIndex((p) => p._id === $user._id) + 1}
				</p>
			</div>
		{/if}
	</div>
