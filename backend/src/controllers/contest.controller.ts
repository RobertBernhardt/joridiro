...
import editContestModel from '../models/editContest.model';
const cloudScheduler = google.cloudscheduler('v1beta1')
const schedule = require('node-schedule');

const convertToCron = (date: Date) => {
...
const uploadImage = async (file: string, id: string, name) => {
    try {
...
const pickLotteryWinner = async (contestID: string) => {
    // > Get the contest
...
const contestController = {
    create: async (req: Request, res: Response) => {
        ...
        try {
            ...
            const totalDays = contestTypes.get(req.body.size).days
            // > Add totalDays to the startDate
            let endDate = new Date(startDate.getTime() + totalDays * 24 * 60 * 60 * 1000)

            const contestObject = contestTypes.get(req.body.size)
            ...
            let index = 0
            // > Use title as the id
            let id = req.body.title.replace(/ /g, '_')
            console.log(id)
            // > If id exists, add a number to the end of it
            while (await contestModel.findById(id)) {
                console.log(index)
                index++
                id = id + index
            }
            // > Create the contest
            const contest: IContest = await contestModel.create({
                _id: id,
                ...
                grandPrize: {
                    amount: contestTypes.get(req.body.size).grand_prize,
                    winner: null
                },
                lotteryPrize: {
                    amount: contestTypes.get(req.body.size).lottery_prize,
                    winner: null
                },
                participants: [],
            ...
            if (req.body.banner[0]) {
                contest.banner = await uploadImage(req.body.banner[0].src, contest._id, 'banner')
            }
            // > Upload the logo
            if (req.body.about_company.logo) {
                contest.about_company.logo = await uploadImage(req.body.about_company.logo, contest._id, 'logo')
            }
            ...
            if (req.body.type === "DEADLINE") {
                ...
                for (let milestone of contest.milestones) {
                    ...
                    const cronForDate = await contestCron.findById(dateWithoutTime)
                    if (cronForDate) {
                        ...
                        const newCron = await contestCron.create({
                            _id: dateWithoutTime,
                            contests: [
                                {
                                    date: milestone.date,
                    ...
                    schedule.scheduleJob(convertToCron(milestone.date), async () => {
                        await endDeadlineMilestone(contest._id, milestone._id)
                    })
                }
                const cronForEndDate = await contestCron.findById(endDateWithoutTime)
                if (cronForEndDate) {
                    console.log("HERE")
                ...
                schedule.scheduleJob(convertToCron(endDate), async () => {
                    await endDeadlineContest(contest._id)
                })
            }
    ...
    edit: async (req: Request, res: Response) => {
        try {
            ...
            if (contest.organizer.toString() !== req.user._id.toString() && !req.user.sys_permissions?.some((permission: string) => ['ADMIN'].includes(permission))) return res.status(401).json({ message: 'UNAUTHORIZED' })
            // > Check if the contest is open
            if (!contest.open) return res.status(400).json({ type: "ERROR", message: "You can't edit a contest that is not open" })
            // > Make sure only one request for a contest is being processed at a time
            const contestExists = await editContestModel.findById(contest._id)
            if (contestExists) return res.status(400).json({ type: "ERROR", message: "You can't edit a contest that is already being edited" })
            // > Add the edited contest to edited_contests
            await editContestModel.create({
                _id: contest._id,
                title: req.body.title,
    ...
    announce: async (req: Request, res: Response) => {
        ...
        try {
            const contest = await contestModel.findById(req.params.id)
            if (!contest) return res.status(404).json({ message: 'INVALID_CONTEST_ID' })
            // > Check if the user is an organizer of the contest or his sys_permissions include ADMIN or CONTEST_ADMIN
            if (contest.organizer.toString() !== req.user._id.toString() && !req.user.sys_permissions?.some((permission: string) => ['ADMIN'].includes(permission))) return res.status(401).json({ message: 'UNAUTHORIZED' })
            // > Check if the contest is over
            if (!contest.open) return res.status(400).json({ message: 'CONTEST_IS_OVER' })
    ...
    announceEdit: async (req: Request, res: Response) => {
        // > Check if the user is an organizer of the contest
        if (!req.user) return res.status(401).json({ message: 'UNAUTHORIZED' })
        try {
            const contest = await contestModel.findById(req.params.id)
            ...
            const announcement = contest.announcements.find((announcement: any) => announcement._id.toString() === req.params.announcementId)
            if (!announcement) return res.status(404).json({ message: 'INVALID_ANNOUNCEMENT_ID' })
            // > Edit announcement
    ...
    announcementDelete: async (req: Request, res: Response) => {
        // > Check if the user is an organizer of the contest
        if (!req.user) return res.status(401).json({ message: 'UNAUTHORIZED' })
        try {
            const contest = await contestModel.findById(req.params.id)
            ...
            if (!announcement) return res.status(404).json({ message: 'INVALID_ANNOUNCEMENT_ID' })
            // > Delete announcement
            contest.announcements = contest.announcements.filter((announcement: any) => announcement._id.toString() !== req.params.announcementId)
            const result = await contest.save()
            return res.status(200).json({ message: 'SUCCESS', data: result })
        } catch (err) {
            return res.status(500).json({ type: "ERROR", message: 'Something went wrong', error: err.message })
    ...
    pickDeadlineContestMilestoneWinner: async (req: Request, res: Response) => {
        try {
    ...
    join: async (req: Request, res: Response) => {
        ...
        // > Check if the user is an organizer of the contest
        if (!req.user) return res.status(401).json({ type: "ERROR", message: 'You are not authorized to join this contest' })
        try {
            const contest = await contestModel.findById(req.params.id)
            if (!contest) return res.status(404).json({ type: "ERROR", message: "Invalid contest" })

            // > Get the user
            const user = await userModel.findById(req.user._id)
            if (!user) return res.status(404).json({ type: "ERROR", message: "You are not logged in" })

            // > Check if the user is an organizer of the contest or his sys_permissions include ADMIN
            if (contest.organizer.toString() === req.user._id.toString()) return res.status(400).json({ type: "ERROR", message: "You can not join your own contest" })

            // > Check if the contest is over
            if (!contest.open) return res.status(400).json({ type: "ERROR", message: "This contest is over" })

            // > Check if the user has already joined the contest
            if (user.contests.includes(contest._id)) return res.status(400).json({ type: "ERROR", message: 'You have already joined the contest' })


            const scoreObj = contest.score.map((score: any) => {
                return {
                    category: score._id,
                    value: 0,
                    points: 0
    ...
    search: async (req: Request, res: Response) => {
        try {
            if (!req.body.query) return res.status(400).json({ type: "ERROR", message: 'Please provide a query' })
            const contests = await contestModel.find({ title: { $regex: req.body.query, $options: 'i' } })
            return res.status(200).json({ type: "SUCCESS", message: 'Contests found', data: contests })
        } catch (err) {
            return res.status(500).json({ type: "ERROR", message: err.message })
        }
    },
    validAlias: async (req: Request, res: Response) => {
        try {
            const contest = await contestModel.findById(req.params.id)
            if (!contest) return res.status(404).json({ type: "ERROR", message: 'Invalid contest id' })
            const participant = contest.participants.find((participant: any) => participant.alias === req.body.alias)
            if (participant) return res.status(400).json({ type: "ERROR", message: 'This alias is already taken' })
    ...
    updateScore: async (req: Request, res: Response) => {
        try {
            ...
            if (!contest) return res.status(404).json({ message: 'INVALID_CONTEST_ID' })
            const user = await userModel.findById(req.user._id)
            if (!user) return res.status(404).json({ message: 'INVALID_USER_ID' })
            ...
            if (contest.organizer.toString() !== req.user._id.toString() && !req.user.sys_permissions.some((permission: string) => ['ADMIN', 'CONTEST_ADMIN'].includes(permission)) && !user.contests.includes(contest._id)) return res.status(401).json({ message: 'UNAUTHORIZED' })
            ...
            if (!contest.score.some((score: any) => score._id.toString() === req.body.score_id)) return res.status(400).json({ message: 'INVALID_SCORE_ID' })
            // > Update the score of the participant for the score id
            contest.participants = contest.participants.map((participant: any) => {
                if (participant._id.toString() === req.user._id.toString()) {
                    participant.score = participant.score.map((score: any) => {
                        console.log(score._id.toString(), req.body.score_id)
                        if (score.category.toString() === req.body.score_id) {
                            score.value = req.body.score
                            const scoreData = contest.score.find((score: any) => score._id.toString() === req.body.score_id)
            ...
            const participant = contest.participants.find((participant: any) => participant._id.toString() === req.user._id.toString())
            if (contest.type === "SCORE") {
                contest.milestones = contest.milestones.map((milestone: any) => {
                    const totalScore = participant.score.reduce((total: number, score: any) => {
                    ...
                    if (milestone.points <= totalScore) {
                        ...
                        if (milestone.participants_reached.includes(req.user._id)) {
                            milestone.participants_reached = milestone.participants_reached.filter((participant: any) => participant.toString() !== req.user._id.toString())
                        }
                    }
                    return milestone
                })
                ...
                if (totalScore >= contestTypes.get(contest.size).max_score) {
                    ...
                    // > Lottery 
                    await pickLotteryWinner(contest._id)
                } else {
                    if (contest.grandPrize.participants_reached.includes(req.user._id)) {
                        contest.grandPrize.participants_reached = contest.grandPrize.participants_reached.filter((participant: any) => participant.toString() !== req.user._id.toString())
                    }
                }
            }
            ...
            contest.participants = contest.participants.sort((a: any, b: any) => {
                let aScore = a.score.reduce((acc: number, score: any) => acc + score.points, 0)
                let bScore = b.score.reduce((acc: number, score: any) => acc + score.points, 0)
    ...
    get: async (req: Request, res: Response) => {
        try {
            // > Get the contest
            let contest = await contestModel.findById(req.params.id)
            if (!contest) return res.status(404).json({ message: 'INVALID_CONTEST_ID' })
            // > Score for the current user
            if (contest.payment_status != "PAID" && contest.organizer.toString() != req.user?._id?.toString()) {
            ...
            let userScore = null
            if (req.user) {
                userScore = contest.participants.find((participant: any) => {
            ...
            if (req.user) {
                if (req.user._id.toString() != contest.organizer.toString()) {
                    contest.participants = contest.participants.map((participant: any, index) => {
                        if (participant._id.toString() === req.user._id.toString()) {
                            rank = index + 1
                        }
                        return participant
    ...
    confirmWinner: async (req: Request, res: Response) => {
        try {
            // > Get the contest
            let contest = await contestModel.findById(req.params.id)
            if (!contest) return res.status(404).json({ message: 'INVALID_CONTEST_ID' })
            // > Check if the user is the organizer of the contest
            if (contest.organizer.toString() !== req.user._id.toString()) return res.status(403).json({ message: 'NOT_AUTHORIZED' })
            ...
            if (['MILESTONE', 'GRAND_PRIZE', 'LOTTERY'].indexOf(winnerType) === -1) return res.status(400).json({ message: 'INVALID_WINNER_TYPE' })
            if (winnerType === "MILESTONE") {
                const milestoneId = req.body.milestoneId
                const milestone = contest.milestones.find((milestone: any) => milestone._id.toString() === milestoneId.toString())
                if (!milestone) return res.status(404).json({ type: 'ERROR', message: 'Milestone not found' })
                // > Check that the score of the winner is greater than the score for the milestone
                const participant = contest.participants.find((participant: any) => participant._id.toString() === req.body.winner.toString())
                const winnerScore = participant.score.reduce((acc: number, score: any) => acc + score.points, 0)
                if (winnerScore < milestone.points) return res.status(400).json({ type: 'ERROR', message: 'The winner does not have the required score to win the milestone. It might be the case that the participant updated their score. Please refresh the page to see the updates' })
                contest.milestones = contest.milestones.map((milestone: any) => {
                    if (milestone._id.toString() === milestoneId.toString()) {
                ...
                if (req.user._id.toString() === contest.organizer.toString()) {
                    contest.participants.map(async (participant: any) => {
                        const participantName = await userModel.findById(participant._id).select('fullName')
    ...
    getParticipantInfo: async (req: Request, res: Response) => {
        try {
            const users = await userModel.find({ _id: { $in: req.body.ids } }, { fullName: 1, _id: 1 })
    ...
    getAll: async (req: Request, res: Response) => {
        try {
            ...
            if (!req.query.page) return res.status(400).json({ message: 'INVALID_PAGE' })
    ...
    faq: async (req: Request, res: Response) => {
        try {
            // > Get the contest
            const contest = await contestModel.findById(req.params.id)
            if (!contest) return res.status(404).json({ message: 'INVALID_CONTEST_ID' })
            // > Check if the contest is open
            if (!contest.open) return res.status(400).json({ type: "ERROR", message: 'Contest is over', data: contest })
            // > Check if the user is the participant
            const participant = contest.participants.find((participant: any) => participant._id.toString() === req.user._id.toString())
            if (!participant && contest.organizer.toString() !== req.user._id) return res.status(404).json({ type: "ERROR", message: 'You are not a participant of this contest', data: contest })
    ...
    answerfaq: async (req: Request, res: Response) => {
        try {
            // > Get the contest
            const contest = await contestModel.findById(req.params.id)
            if (!contest) return res.status(404).json({ message: 'INVALID_CONTEST_ID' })
            ...
            if (req.user._id.toString() !== contest.organizer.toString() && !req.user.sys_permission.includes('CONTEST') && !req.user.sys_permission.includes('ADMIN')) return res.status(401).json({ message: 'UNAUTHORIZED' })
            // > Check if the question exists
            const question = contest.faq.find((question: any) => question._id.toString() === req.params.faqid.toString())
            if (!question) return res.status(404).json({ message: 'INVALID_QUESTION_ID' })
    ...
    deletefaq: async (req: Request, res: Response) => {
        try {
            ...
            if (!contest) return res.status(404).json({ type: "ERROR", message: "Invalid contest" })
            //  > Check if the contest is open
            if (!contest.open) return res.status(400).json({ type: "ERROR", message: 'Contest is over' })
            // > Check if the user is the organiser or if his sys_permission has either of the two values CONTEST OR ADMIN
            if (req.user._id.toString() !== contest.organizer.toString() && !req.user.sys_permission.includes('CONTEST') && !req.user.sys_permission.includes('ADMIN')) return res.status(401).json({ type: "ERROR", message: 'You are not authorized to perform this action' })
            // > Check if the question exists
            const question = contest.faq.find((question: any) => question._id.toString() === req.params.faqid.toString())
            if (!question) return res.status(404).json({ type: "ERROR", message: 'Invalid question id' })
            // > Delete the question
            contest.faq = contest.faq.filter((question: any) => question._id.toString() !== req.params.faqid.toString())
            await contest.save()
    ...
    deadlinecontestcron: async (req, res) => {
        const todayDateWithoutTime = new Date().setHours(0, 0, 0, 0);
        const todaysCron = await cronModel.findById(todayDateWithoutTime)
        if (!todaysCron) return
        // > Convert the date to the cron format
        // > Schedule the cron for the day
        todaysCron.contests.forEach(async (task: any) => {
            console.log(convertToCron(task.date))
            schedule.scheduleJob(convertToCron(task.date), async () => {
                // > If it is a milestone task
                if (task.type === 'MILESTONE') {
                    endDeadlineMilestone(task.contest_id, task._id)
                } else if (task.type === 'CONTEST_END') {
                    const contest = await contestModel.findById(task.contest_id)
                    if (!contest) return
                    contest.open = false
                    await contest.save()
                }
